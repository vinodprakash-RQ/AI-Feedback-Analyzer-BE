...
The asynchronous analysis worker sends feedback to Claude using the Anthropic SDK. Configure `ANTHROPIC_API_KEY` with an API key from the Anthropic Console and set `CLAUDE_MODEL` to a model enabled for your account. The prompt and JSON validation live in `src/services/claude-analysis.service.ts`. The in-process rate limiter is suitable for a single instance only; use a shared Redis/token-bucket implementation before horizontal scaling.
