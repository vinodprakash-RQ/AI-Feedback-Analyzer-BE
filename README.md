...
The asynchronous analysis worker sends feedback to Gemini using the Google Gen AI SDK. Configure `GEMINI_API_KEY` with a key from Google AI Studio and set `GEMINI_MODEL` to a model enabled for your account. The prompt, structured JSON response schema, redaction, and validation live in `src/services/gemini-analysis.service.ts`. Analysis jobs are persisted in PostgreSQL and processed only by `npm run worker:analysis`; deploy that worker separately in production. The worker performs at most 10 Gemini analysis attempts per UTC calendar day. Remaining submissions stay persisted with pending analysis and are picked up on subsequent scheduled worker runs. Each Gemini attempt, including a retry, consumes one of the 10 daily slots. The current in-process rate limiter is suitable for a single instance only; use a shared Redis/token-bucket implementation before horizontal scaling.
